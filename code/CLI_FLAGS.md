...
## Phase D/E/F (Bidirectional Successor) Options
Disabled by default; Phase C remains unchanged unless explicitly enabled.

| Flag | Type / Default | Applies To | Stored In Config | Consumption |
| --- | --- | --- | --- | --- |
| `--phaseD.enabled[=true|false]` | boolean, default `false` | run | `phaseD.enabled` | Enables Phase D (predecessor, -1) training. |
| `--phaseE.enabled[=true|false]` | boolean, default `false` | run | `phaseE.enabled` | Enables Phase E joint (+/-) training. |
| `--phaseE.opSchedule=<alternate|uniform>` | enum, default `uniform` | run (Phase E) | `phaseE.opSchedule` | Operator schedule for Phase E: `uniform` samples plus/minus with p=0.5, `alternate` toggles by trial index. |
| `--phaseE.initFrom=<auto|phaseB|phaseA>` | enum, default `auto` | run (Phase E) | `phaseE.initFrom` | Initialization source for Phase E: `auto` uses Phase D when present, else Phase B/A; `phaseB` forces Phase B snapshot (or Phase A when counting is disabled); `phaseA` forces Phase A snapshot. |
| `--phaseE.transitionSource=<pairs|randomWalk>` | enum, default `pairs` | run (Phase E) | `phaseE.transitionSource` | Transition sampling for Phase E: `pairs` uses independent start digits; `randomWalk` uses local ±1 steps with episode state carryover. |
| `--phaseE.walk.episodeLen=<int>` | integer ≥ 1, default `20` | run (Phase E) | `phaseE.walk.episodeLen` | Number of steps per random-walk episode. |
| `--phaseE.walk.episodesPerBatch=<int>` | integer ≥ 1, default `50` | run (Phase E) | `phaseE.walk.episodesPerBatch` | Episode count per Phase E run batch when using random-walk transitions. |
| `--phaseE.walk.resetStrategy=<uniform|cycle|fixed:<digit>>` | enum, default `uniform` | run (Phase E) | `phaseE.walk.resetStrategy` | Episode reset strategy for random walk (`fixed` accepts a digit 0-9). |
| `--phaseE.walk.bias=<float>` | number in [0,1], default `0.5` | run (Phase E) | `phaseE.walk.bias` | Probability of opPlus when using uniform schedule in random-walk Phase E. |
| `--phaseF.enabled[=true|false]` | boolean, default `false` | run | `phaseF.enabled` | Enables Phase F evaluation for +1 and -1 ops. |
| `--phaseF.splitByOp[=true|false]` | boolean, default `true` when `phaseF.enabled` | run (Phase F) | `phaseF.splitByOp` | When true, emits op-split metrics, confusions, and trace plots. |
| `--phaseF.includeBoundaryMetrics[=true|false]` | boolean, default `true` when `phaseF.enabled` | run (Phase F) | `phaseF.includeBoundaryMetrics` | Adds boundary-only metrics (9→0 for +1, 0→9 for -1). |
| `--phaseF.evalTransitionSource=<pairs|randomWalk>` | enum, default `pairs` | run (Phase F) | `phaseF.evalTransitionSource` | Evaluation transitions for Phase F: `pairs` evaluates all start digits; `randomWalk` evaluates local-walk episodes. |
| `--phaseF.walk.episodeLen=<int>` | integer ≥ 1, default `40` | run (Phase F) | `phaseF.walk.episodeLen` | Steps per random-walk evaluation episode. |
| `--phaseF.walk.episodes=<int>` | integer ≥ 1, default `100` | run (Phase F) | `phaseF.walk.episodes` | Number of evaluation episodes when using random-walk transitions. |
| `--phaseF.walk.resetStrategy=<uniform|fixed:<digit>>` | enum, default `uniform` | run (Phase F) | `phaseF.walk.resetStrategy` | Episode reset strategy for random-walk evaluation (`fixed` accepts a digit 0-9). |
| `--phaseF.walk.bias=<float>` | number in [0,1], default `0.5` | run (Phase F) | `phaseF.walk.bias` | Probability of opPlus when using random-walk evaluation. |
