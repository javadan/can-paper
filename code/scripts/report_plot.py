...
def plot_run(payload: dict, out_dir: str, formats: Sequence[str], topology: str):
  ...
  phase_f = payload.get('phaseF') or {}
  counting_label = 'countingOn' if payload['config'].get('useCountingPhase') else 'countingOff'
  for op_label in ['plus', 'minus']:
    confusion_key = f"confusion{op_label.capitalize()}"
    confusion_f = phase_f.get(confusion_key)
    if confusion_f:
      plot_confusion(
        confusion_f.get('readout', {}),
        f"Phase F readout ({topology}, {op_label})",
        f"confusion_phaseF_readout_{topology}_{op_label}_{counting_label}",
        out_dir,
        formats,
      )
      plot_confusion(
        confusion_f.get('proto', {}),
        f"Phase F proto ({topology}, {op_label})",
        f"confusion_phaseF_proto_{topology}_{op_label}_{counting_label}",
        out_dir,
        formats,
      )
...
def plot_traces(json_path: str, out_dir: str, formats: Sequence[str]):
  ...
  counting_label = 'countingOn' if meta.get('counting') else 'countingOff'
  t_trans = meta.get('tTrans', 0)
  ax.set_title(f"Attractor Stability ({topo}, t={t_trans})")
  ax.axhline(1.0, linestyle='--', color='k', alpha=0.5)

  if operator:
    base_name = f"trace_dynamics_{topo}_{operator}"
  else:
    base_name = f"trace_dynamics_{topo}_{counting_label}"
...
def main():
  ...
  parser.add_argument('--kind', choices=['probe', 'run', 'run_acc_history', 'perturb', 'trace'], required=True)
  ...
  if args.traceFile:
    ...
    raise ValueError('Missing --traceFile for trace-only plotting')
