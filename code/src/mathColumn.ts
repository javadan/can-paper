...
export class MathColumn {
  ...
  private executeTransition(
    ...
    transitionDebugger?.startTrial(startDigit, targetDigit, {
    ...
    try {
      ...
      for (let t = 0; t < this.tTrans; t++) {
        this.step(null, true, currentStats, debugAblations);
  ...
  step(
    ...
    if (useTransition) {
      MathOps.matVecMul(this.W_next, activityVec, this.net.N, this.net.N, this.tmpNext);
  ...
  runSuccessorTrial(
    ...
    this.resetState();
    const nextDigit = (digit + 1) % this.net.numDigits;
    const inputSDR = this.encoder.encode(digit);
    const preStateWindow: Float32Array[] = [];
    const recordPreState = () => {
      const activityVec = this.getActivityVector();
    ...
    if (decision?.abortEpisode) {
      ...
      return { aborted: true, targetDigit: nextDigit };
    }
    if (decision?.settleSteps) {
      for (let i = 0; i < decision.settleSteps; i++) {
        this.step(null, false);
    ...
    if (aborted) {
      if (debugCb || transitionDebugger) {
        ...
        if (this.abortLogCount < limit) {
          console.log(
            `[PhaseC] Trial aborted by sustain gate: start=${digit} target=${nextDigit} tailSilent=${outcome.sustain.tailSilentFrac.toFixed(3)}`,
          );
    ...
    const correct = outcome.predDigit === nextDigit;
    controller?.afterTransition({
      phase: 'PHASE_C_SUCCESSOR',
      trialIdx,
      correct,
