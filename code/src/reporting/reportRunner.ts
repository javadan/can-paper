...
function buildRunTable(result: ExperimentResult): string {
  ...
  const pushRow = (args: {
    ...
    rows.push(
      `| ${[
        args.phase,
        format(args.finalAcc),
        format(args.accPlus),
        format(args.accMinus),
        format(args.boundaryAccPlus),
        format(args.boundaryAccMinus),
        format(args.abortedFrac),
        format(args.sustain?.tailSilentFracMean),
        format(args.sustain?.tailSpikeMassMean),
        format(args.sustain?.timeToSilenceMean),
        format(args.sustain?.gate?.gateFails, 0),
        args.steps ?? 'n/a',
      ].join(' | ')} |`,
    );
  };
  ...
  if (result.phaseB) {
    pushRow({
      phase: 'B',
      finalAcc: result.phaseB.finalAcc,
      abortedFrac: result.phaseB.abortedFrac,
      sustain: result.phaseB.sustain,
      steps: result.phaseB.steps,
    });
  }

  pushRow({
    phase: 'C',
    finalAcc: result.phaseC.finalAcc,
    abortedFrac: result.phaseC.abortedFrac,
    sustain: result.phaseC.sustain,
    steps: result.phaseC.steps,
  });
...
function collectRunFigures(result: ExperimentResult, format: string): ReportFigure[] {
  ...
  if (result.phaseF?.confusionPlus) {
    const countingLabel = result.config.useCountingPhase ? 'countingOn' : 'countingOff';
    figs.push({
      filename: `confusion_phaseF_readout_${result.config.topology}_plus_${countingLabel}.${format}`,
      caption: 'Phase F confusion (readout, +1)',
    });
    figs.push({
      filename: `confusion_phaseF_proto_${result.config.topology}_plus_${countingLabel}.${format}`,
      caption: 'Phase F confusion (prototype, +1)',
    });
  }
  if (result.phaseF?.confusionMinus) {
    const countingLabel = result.config.useCountingPhase ? 'countingOn' : 'countingOff';
    figs.push({
      filename: `confusion_phaseF_readout_${result.config.topology}_minus_${countingLabel}.${format}`,
      caption: 'Phase F confusion (readout, -1)',
    });
    figs.push({
      filename: `confusion_phaseF_proto_${result.config.topology}_minus_${countingLabel}.${format}`,
      caption: 'Phase F confusion (prototype, -1)',
    });
  }
  return figs;
