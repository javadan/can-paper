...
function runTracePlots(
  ...
  const logs: string[] = [];
  let success = true;
  const tracePattern = /trace_(snake|ring)_/;
  for (const traceFile of traceFiles) {
    ...
    const runInput = fs.existsSync(runInputPreferred)
      ...
      : fs.existsSync(runInputFallback)
        ...
        : undefined;
    if (!runInput) {
      logs.push(`[Overnight] Missing run output for trace plotting (${topology}). Expected ${runInputPreferred}.`);
      success = false;
      continue;
    }
...
function writeCombinedReport(
  ...
  if (runArtifacts.length > 0) {
    ...
    runArtifacts.forEach((artifact) => {
      ...
      detailLines.push(
        `- ${artifact.label}: success phaseB=${result.successPhaseB ? 'yes' : 'no'}, phaseC=${result.successPhaseC ? 'yes' : 'no'}, countingPhase=${result.config.useCountingPhase ? 'on' : 'off'}, seed=${result.config.randomSeed}`,
      );
